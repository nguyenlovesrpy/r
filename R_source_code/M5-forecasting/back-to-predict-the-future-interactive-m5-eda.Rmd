...
```{r echo=FALSE}
setwd("C:/Users/DellPC/Desktop/R_source_code/M5-forecasting")
...
foo <- calendar %>% mutate(date = as.Date(date))%>%
  select(date, starts_with("snap")) %>% 
  pivot_longer(starts_with("snap"), names_to = "state", values_to = "snap") %>% 
  mutate(state = str_sub(state, 6,7)) %>% 
...
foo <- train %>%
  ...
  extract_ts() %>% 
  rename(cat_id = id) %>% 
  left_join(calendar %>% mutate(date=as.Date(date)) %>% select(date, event_type_1), by = c("dates" = "date")) %>% 
  filter(!str_detect(as.character(dates), "-12-25")) %>% 
  group_by(cat_id) %>% 
  mutate(loess = predict(loess(sales ~ as.integer(dates - min(dates)) + 1, span = 1/2, degree = 1)),
         mean_sales = mean(sales)) %>% 
...
foo <- train %>%
  ...
  extract_ts() %>% 
  rename(state_id = id) %>% 
  left_join(bar %>% mutate (date= as.Date(date)), by = c("dates" = "date", "state_id")) %>% 
  filter(!str_detect(as.character(dates), "-12-25")) %>% 
  mutate(snap = as.logical(snap)) %>% 
  group_by(state_id) %>% 
  mutate(loess = predict(loess(sales ~ as.integer(dates - min(dates)) + 1, span = 1/2, degree = 1)),
...
calendar<- calendar %>% mutate (date= as.Date(date))
