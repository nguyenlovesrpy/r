...
Below, I am reading the csv's dataframes into R 
...
Without the Id's, teh dataframe consists of 79 predictors and our response variable SalePrice 
...
# Misssing data, label encoding, and factorizing variables 
...
Of course, teh 1459 NA's in SalePrice match the size of the test set perfectly. This means that I have to fix NAs in 34 predictor variables.
...
It is clear that I can label encode this vriable as the values are ordinal. As there a multiple variables that use the same quality levels, I am going to create a vector that I can reuse later on.
...
When looking at the frequencies, the variable seems irrelevant to me. Having a shed probably means to Garage, which would explain the lower sales price for Shed. Also, while it makes a lot of sense that a house with Tennies court is expensive, there is only one house with a tennies court in the training set
...
As NAs mean 'No Garage' for character variables, I now want to find out where the differences between 157 NA GarageType and the other 3 character variables with 159 NAs come from.
